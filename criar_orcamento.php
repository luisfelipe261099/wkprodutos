...
function obterOpcoesEnumDaColuna($conexao, $tabela, $coluna) {
    ...
    $conteudoEnum = $matches[1];
    $valores = str_getcsv($conteudoEnum, ',', "'", "\\");

    $opcoes = array_values(array_filter(array_map(static function ($valor) {
        $valorLimpo = trim((string) $valor);
        $valorLimpo = trim($valorLimpo, "'\" ");
        return str_replace("\\'", "'", $valorLimpo);
    }, $valores), static function ($valor) {
        return $valor !== '';
    }));
...
if (count($tipos_faturamento_permitidos) === 1 && strpos($tipos_faturamento_permitidos[0], ',') !== false) {
    $tipos_faturamento_permitidos = array_values(array_filter(array_map('trim', explode(',', str_replace("'", '', $tipos_faturamento_permitidos[0])))));
}
        ...
        <form id="formOrcamento" method="post">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-4">
                    ...
                    <select id="tipo_faturamento_mobile" class="form-control mobile-billing-select" aria-label="Tipo de faturamento no celular">
                        <?php foreach ($tipos_faturamento_permitidos as $opcao_tipo): ?>
                            <option value="<?php echo htmlspecialchars($opcao_tipo); ?>" <?php echo $tipo_faturamento === $opcao_tipo ? 'selected' : ''; ?>><?php echo htmlspecialchars($labels_tipo_faturamento[$opcao_tipo] ?? $opcao_tipo); ?></option>
                        <?php endforeach; ?>
                    </select>
...
const tipoFaturamentoMobileSelect = document.getElementById('tipo_faturamento_mobile');
...
function sincronizarTipoFaturamento(origem, destino) {
    if (!origem || !destino) {
        return;
    }
...
if (tipoFaturamentoSelect && tipoFaturamentoMobileSelect) {
    sincronizarTipoFaturamento(tipoFaturamentoSelect, tipoFaturamentoMobileSelect);

    tipoFaturamentoSelect.addEventListener('change', function() {
        sincronizarTipoFaturamento(tipoFaturamentoSelect, tipoFaturamentoMobileSelect);
    });

    tipoFaturamentoMobileSelect.addEventListener('change', function() {
        sincronizarTipoFaturamento(tipoFaturamentoMobileSelect, tipoFaturamentoSelect);
    });
}
</script>
