...
// LÃ³gica para ExclusÃ£o de TransaÃ§Ã£o
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    if ($stmt_delete = $conn->prepare($sql_delete)) {
        ...
        if ($stmt_delete->execute()) {
            ...
            $message = "Erro ao excluir a transaÃ§Ã£o: " . $stmt_delete->error;
            $message_type = "danger";
        }
        $stmt_delete->close();
    }
...
// --- NOVOS CÃLCULOS FINANCEIROS GLOBAIS ---

// 1. Calcular Receita Bruta Total (todas as vendas concluÃ­das)
...
// 3. Calcular Entradas e SaÃ­das Manuais (da tabela transacoes_financeiras)
$outras_entradas = 0;
$total_despesas = 0;
...
// 4. Calcular o Saldo LÃ­quido Final - CORREÃ‡ÃƒO: Lucro das Vendas menos Despesas
$saldo_liquido = $lucro_total_vendas - $total_despesas;

// LÃ³gica de paginaÃ§Ã£o para transaÃ§Ãµes
...
// LÃ³gica para buscar o histÃ³rico de transaÃ§Ãµes para a tabela (paginado)
...
<div class="page-header fade-in-up">
    ...
    <p class="page-subtitle">
        Acompanhe a receita, lucro, despesas e o fluxo de caixa do seu negÃ³cio.
    </p>
</div>
...
<div class="row g-4 mb-4 fade-in-up">
    ...
    <div class="col-6 col-lg-3">
        <div class="stats-card danger">
            ...
            <div class="stats-label">Despesas (SaÃ­das)</div>
        </div>
    </div>
    ...
    <div class="col-6 col-lg-3">
        <div class="stats-card <?php echo ($saldo_liquido >= 0 ? 'info' : 'warning'); ?>">
            ...
            <div class="stats-label">Saldo LÃ­quido</div>
        </div>
    </div>
</div>
...
<div class="modern-card fade-in-up">
    <div class="card-header-modern">
        ...
        HistÃ³rico de TransaÃ§Ãµes Manuais
        <div class="ms-auto">
            <a href="registrar_transacao.php" class="btn btn-primary btn-sm">
                <i class="fas fa-plus me-1"></i> Nova TransaÃ§Ã£o Avulsa
            </a>
        </div>
    </div>
        ...
        <?php if ($result_transacoes && $result_transacoes->num_rows > 0): ?>
                        ...
                        while($row = $result_transacoes->fetch_assoc()) {
                            ...
                            if (!empty($row['referencia_id']) && !empty($row['tabela_referencia'])) {
                                ...
                                if ($tabela_ref == 'vendas') {
                                    ...
                                    $referencia_link = '<a href="detalhes_orcamento.php?id=' . $ref_id . '" class="text-decoration-none" title="Ver OrÃ§amento"><i class="fas fa-file-alt"></i> OrÃ§am. #' . $ref_id . '</a>';
                                }
                            }
                            ?>
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="btn-group" role="group">
                                        ...
                                        <a href="financeiro.php?action=delete&id=<?php echo $row['id']; ?>" class="btn btn-outline-danger btn-sm" title="Excluir" onclick="return confirm('Tem certeza que deseja excluir esta transaÃ§Ã£o? Isso pode afetar o balanÃ§o financeiro.');">
            ...
            <?php if ($total_paginas_transacoes > 1): ?>
                ...
                echo render_pagination([
                    ...
                    'summary' => $total_transacoes_registros . ' transaÃ§Ãµes registradas',
                    'aria_label' => 'NavegaÃ§Ã£o de transaÃ§Ãµes financeiras'
                ]);
                ?>
            <?php endif; ?>
        <?php else: ?>
            <div class="text-center py-5">
                ...
                <h5 class="text-muted mb-2">Nenhuma transaÃ§Ã£o manual registrada</h5>
                <p class="text-muted">Registre despesas ou outras entradas para vÃª-las aqui.</p>
                <a href="registrar_transacao.php" class="btn btn-primary">
                    <i class="fas fa-plus me-2"></i> Registrar Primeira TransaÃ§Ã£o
                </a>
            </div>
        <?php endif; ?>
    </div>
